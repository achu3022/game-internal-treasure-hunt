...
class Board {
  ...
  isSolved() {
    if (this.state[this.size - 1][this.size - 1] === 0) {
        if (this.goal_state.flat().toString() == this.state.flat().toString()) {
            this.solved = true;
            let completionTime = new Date().toISOString();  // Store ISO date format
            let moves = $(".move").children("span").text(); // Get number of moves
            let userId = $("#user_id").val(); // Get logged-in user's ID (if available)
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log("Game result stored:", response);
                    alert("🎉 You are the winner! Your result has been saved.");
                },
...
playAgainPromt() {
  let board = $(".board");
  let userId = $("#user_id").val(); // Ensure this exists
  let moves = parseInt($(".move").children("span").text(), 10) || 0;
  ...
  if (!userId) {
      alert("Error: Missing User ID. Redirecting to login...");
  ...
  $.ajax({
      ...
      success: function(response) {
          ...
          if (response.error) {
              ...
              // Ensure #0 exists before modifying opacity
              if ($("#0").length) {
                  setTimeout(() => {
                      $("#0").css("opacity", "1");
                  }, 500);
              ...
              // Create the "Play Again" button only if board exists
              if (board.length) {
                $("<button/>", {
                    html: "<span>🎉 You Win! Logout?</span>",
      ...
      error: function(xhr, status, error) {
          console.error("Error storing game result:", xhr.responseText);
      }
  });
}
